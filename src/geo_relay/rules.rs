use crate::runtime_config::{GeoConfig, GeoRuleConfig};
...
impl<'a> ExpressionParser<'a> {
    fn parse(source: &'a str) -> Result<Expression, String> {
        if source.len() > MAX_EXPRESSION_BYTES {
            return Err(format!(
                "expression must not exceed {MAX_EXPRESSION_BYTES} bytes"
            ));
        }
        let mut parser = Self {
            source,
    ...
    fn parse_primary(&mut self) -> Result<Expression, String> {
        ...
        if self.consume('(') {
            if self.depth >= MAX_EXPRESSION_DEPTH {
                return Err(format!(
                    "expression nesting must not exceed {MAX_EXPRESSION_DEPTH} levels"
                ));
            }
            self.depth += 1;
        ...
        let expression = Predicate::compile(raw)?;
        self.add_node()?;
        Ok(expression)
    }
    ...
    fn add_node(&mut self) -> Result<(), String> {
        self.nodes = self
            .nodes
            .checked_add(1)
            .ok_or_else(|| "expression is too complex".to_owned())?;
        if self.nodes > MAX_EXPRESSION_NODES {
            return Err(format!(
                "expression must not exceed {MAX_EXPRESSION_NODES} AST nodes"
            ));
        }
        Ok(())
    }
...
mod tests {
    use super::*;
    use crate::runtime_config::{EndpointExpressions, GeoRuleConfig};
    ...
    #[test]
    fn rejects_oversized_or_deep_expressions() {
        let oversized = "C".repeat(MAX_EXPRESSION_BYTES + 1);
        assert!(ExpressionParser::parse(&oversized).is_err());
        ...
        assert!(ExpressionParser::parse(&deep).is_err());

        let complex = std::iter::repeat_n("CN", 130).collect::<Vec<_>>().join("/");
        assert!(ExpressionParser::parse(&complex).is_err());
    }
}
